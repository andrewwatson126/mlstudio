...
const EnhancedTable2 = props => {
  const { headCells, correlationList, rowClickHander } = props
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
            ...
            <TableBody>
              {stableSort(correlationList, getComparator(order, orderBy))
                ...
                .map((correlation, index) => {
                  ...
                  return (
                    <TableRow hover role="checkbox" tabIndex={-1} key={index} onClick={() => rowClickHander(page * rowsPerPage + index)}>
                      <StyledTableCell    align="right">{correlation.thresholds}</StyledTableCell >
                      <StyledTableCell  align="right">{correlation.sensitivity}</StyledTableCell >
                      <StyledTableCell  align="right">{correlation.specificity}</StyledTableCell >
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
        ...
        <TablePagination rowsPerPageOptions={[10, 25, 100]} component="div" count={correlationList.length} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} />
      </Paper>
      <FormControlLabel
